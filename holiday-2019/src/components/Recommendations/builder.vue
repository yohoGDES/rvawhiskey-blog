	...
	<div v-if="!loading">
		...
		<div class="list-filters">
			...
			<div class="list-filter">
				...
				<label for="" class="inline">Virginia Made Only</label>
			</div>
			<!-- <div class="list-filter">
				<input type="checkbox" v-model="includeOutOfState" :value="!includeOutOfState">
				<label for="" class="inline">Show Out of State Items</label>
			</div> -->
		</div>
		<div>
		...
		<ul class="recommendation-list" v-if="filteredData">
			<li v-for="whiskey in filteredData" :key="whiskey.index" class="key-block" :class="'key-block--' + $util.keySymbol(whiskey.type)">
				...
				<div class="">
					<input type="text" v-model="whiskey.name"> <br/>
					<strong><label for="">Notes</label></strong> <br/>
					<textarea name="" v-model="whiskey.notes" id="" cols="30" rows="10"></textarea>
				</div>
...
export default {
	...
	computed: {
		...
		priceRange() {
			...
			let a = { title: 'Affordable & Available Must Haves', name: '$', value: '(< 25)', low: 0, high: 25.99 }
			let b = { title: 'Step Your Game Up', name: '$$', value: '(25-50)', low: 25, high: 49.99 }
			let c = { title: 'The Sweet Spot', name: '$$$', value: '(50-80)', low: 50, high: 79.99 }
			let d = { title: 'Most Impressive', name: '$$$$', value: '(80-150)', low: 80, high: 149.99 }
			let e = { title: 'Topshelf Afficianado', name: '$$$$$', value: '(150+)', low: 150, high: 9999 }
